...
*/
define ('VERSION', '2.9.0'); // Luca Montanari (aka LCweb) added exif roatation fix in v2.9.0
...
if(isset($_GET['lc-debug'])) {echo 'Website path: '.$true_path.'<br/>';} // lcweb debug
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
        ...
        $image = $this->fix_exif_orientation($localImage, $image);
	...
	protected function calcDocRoot(){
		...
		/////////////////////////////////////////////////////////
		// MULTISITE FIX by (part 2 of 3)
		if (is_multisite()){
			
			// custom path method (2012-11-28)
			$wpmu_path = get_option('mg_wpmu_path');
			if($wpmu_path) {
				$this->debug(3, "Doc root is: " . $wpmu_path);
				$this->docRoot = $wpmu_path;	
				return true;
			}
	...
	protected function getLocalImagePath($src){
		...
		if (is_multisite()){
			...
			if($wpmu_path) {
				...
				$year = $url_array[count($url_array) - 3];
				$month = $url_array[count($url_array) - 2];
				
				// replace placeholders
				$wpmu_path = str_replace(
					array('YEAR', 'MONTH'), 
					array($year, $month),
					$wpmu_path
				);

				$wpmu_path = $wpmu_path.'/'.$filename;
				
				if (file_exists ($wpmu_path)) {
					return $wpmu_path;
				}	
			}
			
			
			$start = ( count($url_array) - 1);
			$part = '';
			for( $a=$start; $a > ($start - 3); $a-- ) {
				$part = '/'.$url_array[$a] . $part;
			}

			$path = $this->docRoot . '/files' . $part;
			return $path;
    ...
    protected function fix_exif_orientation($src, $image) {
