! function (t, e, i, o, n) {
    t.fn.multiscroll = function (s) {
        ...
        function triggerFixedContentAnimations(activeIndex) {

            var $fixedTemplates = t("#premium-multiscroll-" + s.id + " .premium-multiscroll-fixed-temp");

            $fixedTemplates.removeClass('animated ' + s.entranceAnimation).addClass('premium-addons__v-hidden');

            $fixedTemplates.eq(activeIndex).addClass('animated ' + s.entranceAnimation).removeClass('premium-addons__v-hidden');
        ...
        function B(e) {
            ...
            if (s.menu) {
                t(s.menu).find(".active").removeClass("active"); // Remove active class from any previously active menu item
                t(s.menu).find('[data-menuanchor="' + u + '"]').addClass("active"); // Add active class to the menu item corresponding to the current anchor (u)
            }

            // Update d and p variables
            d = r;
            p = n;


            triggerFixedContentAnimations(p);

            // Update navigation if it exists
            if (s.navigation) {
                t("#multiscroll-nav-" + s.id).find(".active").removeClass("active"); // Remove active class from any previously active navigation item

                if (d) { // If d (the anchor) exists
                    t("#multiscroll-nav-" + s.id).find('a[href="#' + d + '"]').addClass("active"); // Add active class to the navigation item with href matching d
                } else {
                    t("#multiscroll-nav-" + s.id).find("li").eq(p).find("a").addClass("active"); // Add active class to the navigation item corresponding to index p
                }
            }
        }
